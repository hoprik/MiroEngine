...
import ru.hoprik.storymod.Story.Engine.AnimationRender.Timecore.client.render.model.loading.JsonModelParser;
import ru.hoprik.storymod.Story.Engine.AnimationRender.Timecore.client.render.model.loading.TimeModelDefinition;
import ru.hoprik.storymod.Story.Engine.AnimationRender.Timecore.internal.client.handlers.ClientLoadingHandler;
import ru.hoprik.storymod.Story.Engine.Mixins.ClientModLoaderAccessor;
...
public class TimeModelSet implements ResourceManagerReloadListener {
    ...
    public TimeModelPart bakeRoot(InFileLocation location) {
        ...
        TimeModelDefinition definition = this.roots.get(InFileLocation.wildcarded(location.location()));

        this.roots.forEach((inFileLocation, timeModelDefinition) -> {
            StoryMod.logger.info(timeModelDefinition);
        });

        if (definition == null) {
            throw new IllegalArgumentException("No model was found with location " + definition.toString());
        } else {
