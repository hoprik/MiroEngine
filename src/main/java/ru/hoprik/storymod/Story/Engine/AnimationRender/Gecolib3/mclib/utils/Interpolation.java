...
public enum Interpolation {
    ...
    EXP_IN("exp_in") {
        public float interpolate(float a, float b, float x) {
            return a + (b - a) * (float)Math.pow(2.0, (double)(10.0F * (x - 1.0F)));
        }
    },
    EXP_OUT("exp_out") {
        public float interpolate(float a, float b, float x) {
            return a + (b - a) * (float)(-Math.pow(2.0, (double)(-10.0F * x)) + 1.0);
        }
    },
    EXP_INOUT("exp_inout") {
        public float interpolate(float a, float b, float x) {
            if (x == 0.0F) {
                ...
                x *= 2.0F;
                if (x < 1.0F) {
                    return a + (b - a) / 2.0F * (float)Math.pow(2.0, (double)(10.0F * (x - 1.0F)));
                } else {
                    --x;
                    return a + (b - a) / 2.0F * (float)(-Math.pow(2.0, (double)(-10.0F * x)) + 2.0);
                }
            }
        }
    };
