...
public abstract class AnimatedLivingEntityRenderer<T extends LivingEntity & AnimatedObject<T>, M extends TimeEntityModel<T>> extends LivingEntityRenderer<T, M> implements ITimeModelRenderer<T>, RenderLayerParent<T, M> {
    ...
    public AnimatedLivingEntityRenderer(EntityRendererProvider.Context ctx, M entityModelIn, float shadowSizeIn) {
        ...
        renderer = ctx.getItemInHandRenderer();
    }

    @Override
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource buffer, int packedLight) {
        getModel().reset();
        renderer.renderItem(entity, entity.getItemBySlot(EquipmentSlot.MAINHAND), ItemTransforms.TransformType.HEAD, false, matrixStack, buffer, packedLight);
